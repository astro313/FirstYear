...
SFE\tnh                   & \Lsun\ (\LpU)\pmOne & 256 $\pm$ 41      & 340 $\pm$ 40          &  R11 & 135 $\pm$ 20\tni   & --- \\
$M_{\rm dyn}$             & 10$^{10}$ \Msun     & 7.8 $\pm$ 2.8     & 3.7 $\pm$ 1.8\tni\tnj &  --- & 6.0 $\pm$ 0.5      & S11 \\
$f_{\rm gas-dyn}$         &                     & 0.4 $\pm$ 0.1     & 0.9\tni               &  --- & 0.6 $\pm$ 0.1      & S11 \\
\enddata
\label{tab:comapreSMG}
\tablecomments{Values listed from rows 6 onwards are lensing-corrected}
\tablenotetext{a}{H14}
\tablenotetext{b}{\CO}
\tablenotetext{c}{Based on a Gaussian fit to the continuum-subtracted line profile}
\tablenotetext{d}{Estimated from Figure 1 in D11}
\tablenotetext{e}{Errors include uncertainties in $\mu_{\rm L}$}
\tablenotetext{f}{Chabrier IMF}
\tablenotetext{g}{Converted the values derived using Salpeter IMF to Chabrier IMF}
\tablenotetext{h}{Independent of lensing magnification factor $\mu_{\rm L}$}
\tablenotetext{i}{We derive this from the reported values}
\tablenotetext{j}{Using CO($J$ = 5 \rarr\ 4)}
