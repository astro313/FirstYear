...
#####################
#####################
sig_LprimeCO = (3.25e7 / freq_CO32**2 * dl**2 / (1+z)) * ((del_I/mu)**2 + (del_mu/mu**2*I_line)**2)**(.5)
del_LprimeCO = 3.25e7 / freq_CO32**2 * dl**2 / (1+z) * ((del_I/mu) + (del_mu/mu**2*I_line))

sig_LprimeCO_noMu = (3.25e7 / freq_CO32**2 * dl**2 / (1+z)) * (del_I/mu)
...
sig_Mdyn = 2.8e5 * ((2. * vel * del_vel * rkpc)**2 + (vel**2 * del_kpc)**2)**(.5)
del_Mdyn = 2.8e5 * (2. * vel * del_vel * rkpc + vel**2 * del_kpc)

sig_SFR = 1.e-10 * ((LIR/mu**2*del_mu)**2 + (del_IR_noMu/mu)**2)**(.5)
del_SFR = 1.e-10 * (LIR/mu**2*del_mu + del_IR_noMu/mu)

sig_SFR_FIR = 1.e-10 * ((LFIR/mu**2*del_mu)**2 + (del_FIR_noMu/mu)**2)**(.5)
